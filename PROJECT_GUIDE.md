...
- 命令入口以 `justfile` 为主，前端命令在 `frontend/package.json`。
...
**构建与测试命令（来自仓库）**
- 后端运行：`just run`（`justfile:10`）或 `go run ./cmd/server`（`justfile:12`）。
- 后端测试：`just test`（`justfile:15`）或 `go test ./... -v`（`justfile:17`）。
- 前端开发：`bun run dev`（`frontend/package.json:8`）。
- 前端构建：`bun run build`（`frontend/package.json:9`）。
...
2. **问**：为什么要在启动时执行 `godotenv.Load()`？
   **答**：为了让 `go run/just run` 直接读取本地 `.env`，同时不覆盖系统环境变量；见注释 `cmd/server/main.go:21` 到 `cmd/server/main.go:25`。
...
| 天数 | 每天目标 | 建议阅读文件 | 验收标准 |
|---|---|---|---|
| Day 1 | 跑通后端与前端，记住入口命令 | `justfile`、`cmd/server/main.go`、`frontend/vite.config.js` | 可独立启动前后端并解释端口/代理关系 |
